...
#define GPIO_AF_SPI3    GPIO_AF_6
...
#ifdef XMC4700_F100x2048
    {
        ...
        .reg = SPI2,
        .sckPins = {
            { DEFIO_TAG_E(P24), XMC_GPIO_MODE_OUTPUT_PUSH_PULL_ALT2, 0 },
            { DEFIO_TAG_E(P30), XMC_GPIO_MODE_OUTPUT_PUSH_PULL_ALT2, 0 },
			{ DEFIO_TAG_E(P36), XMC_GPIO_MODE_OUTPUT_PUSH_PULL_ALT4, 0 },
        },
        .misoPins = {
            { DEFIO_TAG_E(P22), XMC_GPIO_MODE_INPUT_TRISTATE, 0 },
            { DEFIO_TAG_E(P25), XMC_GPIO_MODE_INPUT_TRISTATE, 1 },
			{ DEFIO_TAG_E(P40), XMC_GPIO_MODE_INPUT_TRISTATE, 4 },
        },
        .mosiPins = {
            { DEFIO_TAG_E(P25), XMC_GPIO_MODE_OUTPUT_PUSH_PULL_ALT2, 0 },
            { DEFIO_TAG_E(P35), XMC_GPIO_MODE_OUTPUT_PUSH_PULL_ALT4, 0 },
        },
    },
    {
    	//SLAVE
		...
		.rxIrq = USIC1_0_IRQn,
		.txIrq = USIC1_1_IRQn,
		.rxPriority = NVIC_PRIO_SERIALSPIS3,
		.txPriority = NVIC_PRIO_SERIALSPIS3,
        .sckPins = {
            { DEFIO_TAG_E(P011), XMC_GPIO_MODE_INPUT_TRISTATE, 0 },
            { DEFIO_TAG_E(P58), XMC_GPIO_MODE_INPUT_TRISTATE, 1 },
        },
        .misoPins = {
            { DEFIO_TAG_E(P05), XMC_GPIO_MODE_OUTPUT_PUSH_PULL_ALT2, 0 },
            { DEFIO_TAG_E(P115), XMC_GPIO_MODE_OUTPUT_PUSH_PULL_ALT4, 0 },
			{ DEFIO_TAG_E(P214), XMC_GPIO_MODE_OUTPUT_PUSH_PULL_ALT2, 0 },
        },
        .mosiPins = {
            { DEFIO_TAG_E(P04), XMC_GPIO_MODE_INPUT_TRISTATE, 0 },
            { DEFIO_TAG_E(P05), XMC_GPIO_MODE_INPUT_TRISTATE, 1 },
			{ DEFIO_TAG_E(P215), XMC_GPIO_MODE_INPUT_TRISTATE, 2 },
			{ DEFIO_TAG_E(P214), XMC_GPIO_MODE_INPUT_TRISTATE, 3 },
			{ DEFIO_TAG_E(P114), XMC_GPIO_MODE_INPUT_TRISTATE, 4 },
        },
        .csPins = {
            { DEFIO_TAG_E(P06), XMC_GPIO_MODE_INPUT_TRISTATE, 0 },
            { DEFIO_TAG_E(P59), XMC_GPIO_MODE_INPUT_TRISTATE, 1 },
        },
    },
#endif
};
...
void spiPinConfigure(const spiPinConfig_t *pConfig)
{
    for (size_t hwindex = 0 ; hwindex < ARRAYLEN(spiHardware) ; hwindex++) {
...
#ifdef XMC4700_F100x2048
                pDev->sckAF = hw->sckPins[pindex].af;
                pDev->sckSRC = hw->sckPins[pindex].src;
#endif
            }
...
#ifdef XMC4700_F100x2048
                pDev->misoAF = hw->misoPins[pindex].af;
                pDev->misoSRC = hw->misoPins[pindex].src;
#endif
            }
...
#ifdef XMC4700_F100x2048
                pDev->mosiAF = hw->mosiPins[pindex].af;
                pDev->mosiSRC = hw->mosiPins[pindex].src;
#endif
            }

#ifdef XMC4700_F100x2048
            if (pConfig[device].ioTagCs == hw->csPins[pindex].pin) {
            	pDev->csAF = hw->csPins[pindex].af;
            	pDev->csSRC = hw->csPins[pindex].src;
            }
#endif
        }
        ...
        if (pDev->sck && pDev->miso && pDev->mosi) {
            ...
            pDev->rxIrq = hw->rxIrq;
            pDev->txIrq = hw->txIrq;
            pDev->rxPriority = hw->rxPriority;
            pDev->txPriority = hw->txPriority;
#endif
        }
    }
}
