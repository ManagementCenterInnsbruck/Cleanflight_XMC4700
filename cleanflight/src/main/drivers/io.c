...
int IO_GPIOPortIdx(IO_t io)
{
    ...
    return (((size_t)IO_GPIO(io) - PORT0_BASE) >> 8);     // ports are 0x400 apart
#else
...
#ifdef XMC4700_F100x2048
    return XMC_GPIO_GetInput(IO_GPIO(io), IO_Pin(io));
#else
...
#ifdef XMC4700_F100x2048
    XMC_GPIO_SetOutputLevel(IO_GPIO(io), IO_Pin(io), hi);
#else
#if defined(USE_HAL_DRIVER)
...
#ifdef XMC4700_F100x2048
    XMC_GPIO_SetOutputHigh(IO_GPIO(io), IO_Pin(io));
#else
#if defined(USE_HAL_DRIVER)
...
#ifdef XMC4700_F100x2048
    XMC_GPIO_SetOutputLow(IO_GPIO(io), IO_Pin(io));
#else
#if defined(USE_HAL_DRIVER)
...
#ifdef XMC4700_F100x2048
    XMC_GPIO_ToggleOutput(IO_GPIO(io), IO_Pin(io));
#else
...
void IOConfigGPIO(IO_t io, ioConfig_t cfg)
{
	...
	XMC_GPIO_OUTPUT_LEVEL_t level;

	if (cfg & 0x01)
		level = XMC_GPIO_OUTPUT_LEVEL_HIGH;
	else
		level = XMC_GPIO_OUTPUT_LEVEL_LOW;
	...
	XMC_GPIO_Init(IO_GPIO(io), IO_Pin(io), &config);
}
...
#ifdef XMC4700_F100x2048
            	ioRec->gpio = (GPIO_TypeDef *)(PORT0_BASE + (port << 8));    // ports are 0x100 apart
            	ioRec->pin = pin;
#else
