...
static void pwmOCConfig(TIM_TypeDef *tim, uint8_t channel, uint16_t value, uint8_t output)
{
	...
	XMC_CCU4_SLICE_SetTimerCompareMatch(tim, value);
	XMC_CCU4_EnableShadowTransfer(TIM_MODULE(tim), 1 << 4  *TIM_SLICE_NO(tim));
	XMC_CCU4_SLICE_SetPassiveLevel(tim, (output & TIMER_OUTPUT_INVERTED) ? XMC_CCU4_SLICE_OUTPUT_PASSIVE_LEVEL_LOW : XMC_CCU4_SLICE_OUTPUT_PASSIVE_LEVEL_HIGH);
#else
...
void pwmOutConfig(timerChannel_t *channel, const timerHardware_t *timerHardware, uint32_t hz, uint16_t period, uint16_t value, uint8_t inversion)
{
    ...
    XMC_CCU4_EnableClock(TIM_MODULE(timerHardware->tim), TIM_SLICE_NO(timerHardware->tim));
    XMC_CCU4_SLICE_StartTimer(timerHardware->tim);
#else
#if defined(USE_HAL_DRIVER)
...
static void pwmCompleteXMCMotorUpdate(uint8_t motorCount)
{
	for (int index = 0; index < motorCount; index++) {
		XMC_CCU4_EnableShadowTransfer(TIM_MODULE(motors[index].channel.tim), 1 << 4  *TIM_SLICE_NO(motors[index].channel.tim));
	}
}
...
{
#ifdef XMC4700_F100x2048
	pwmCompleteXMCMotorUpdate(motorCount);
#endif
    for (int index = 0; index < motorCount; index++) {
...
void motorDevInit(const motorDevConfig_t *motorConfig, uint16_t idlePulse, uint8_t motorCount)
{
...
#ifdef XMC4700_F100x2048
        pwmCompleteWrite = useUnsyncedPwm ? &pwmCompleteXMCMotorUpdate : &pwmCompleteOneshotMotorUpdate;
#else
    ...
    for (int motorIndex = 0; motorIndex < MAX_SUPPORTED_MOTORS && motorIndex < motorCount; motorIndex++) {
        ...
        while ((clock / (prescaler * pwmRateHz)) - 1 > 0xffff)
        	prescaler*=2;
#else
