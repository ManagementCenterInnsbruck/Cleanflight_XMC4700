...
#if !defined(USE_INVERTER) && !defined(STM32F303xC) && !defined(XMC4700_F100x2048)
    UNUSED(uartPort);
#else
...
{
#ifdef XMC4700_F100x2048
	XMC_USIC_CH_SetMode((XMC_USIC_CH_t*)uartPort->USARTx, XMC_USIC_CH_OPERATING_MODE_IDLE);

	XMC_UART_CH_CONFIG_t uart_config =
	{
		.baudrate 		= uartPort->port.baudRate,
		.data_bits		= (uartPort->port.options & SERIAL_PARITY_EVEN) ? 9 : 8,
		.frame_length 	= (uartPort->port.options & SERIAL_PARITY_EVEN) ? 9 : 8,
		.stop_bits		= (uartPort->port.options & SERIAL_STOPBITS_2) ? 2 : 1,
		.oversampling	= 16,
		.parity_mode	= XMC_USIC_CH_PARITY_MODE_NONE,
	};
	XMC_UART_CH_Init((XMC_USIC_CH_t*)uartPort->USARTx, &uart_config);
	XMC_USIC_CH_SetInputSource((XMC_USIC_CH_t*)uartPort->USARTx, XMC_USIC_CH_INPUT_DX0, uartPort->source);
