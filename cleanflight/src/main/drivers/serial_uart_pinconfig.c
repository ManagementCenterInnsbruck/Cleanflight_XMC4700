...
void uartPinConfigure(const serialPinConfig_t *pSerialPinConfig)
{
    ...
    for (size_t hindex = 0; hindex < UARTDEV_COUNT; hindex++) {
        ...
        for (int pindex = 0 ; pindex < UARTHARDWARE_MAX_PINS ; pindex++) {
            if (hardware->rxPins[pindex].ioTag && (hardware->rxPins[pindex].ioTag == pSerialPinConfig->ioTagRx[device]))
            {
                uartdev->rx = pSerialPinConfig->ioTagRx[device];
#ifdef XMC4700_F100x2048
                uartdev->rxSrc = hardware->rxPins[pindex].src;
#endif
            }

            if (hardware->txPins[pindex].ioTag && (hardware->txPins[pindex].ioTag == pSerialPinConfig->ioTagTx[device]))
            {
